...
const Portfolio = () => {
    return (
        <Container>
            <div className="w-full py-24 relative" id='portfolio'>
                ...
                <EachUtils dataList={DATA_PORTFOLIO} render={(item, index) => <a target='_blank' href={item.link_github} className={`w-full block md:flex items-center justify-center ${index != DATA_PORTFOLIO.length - 1 ? 'mb-20' : ''}`} key={index}>
                    <div className='font-roboto mb-3'>
                        ...
                        <div className={`bg-white shadow-cust-shad-white shadow-white bg-opacity-20 backdrop-blur-md rounded-md text-white p-4 z-20 relative ${(index + 1) % 2 == 0 ? 'md:-translate-x-5' : 'md:translate-x-5'}`}>
                            <p className='text-sm'>
                                {item.description}
                            </p>
                        </div>
                    </div>
                    <img src={item.image} alt="Image Portfolio" className={`w-full md:w-1/2 h-60 z-10 relative rounded-md shadow-cust-shad-secondary object-cover  ${(index + 1) % 2 == 0 ? 'md:-order-1 md:translate-x-5' : 'order-1 md:-translate-x-5'}`} />
                </a>} />
            </div>
        </Container>
    )
